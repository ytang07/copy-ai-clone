...
class PromptPackage(PackageService):
  ...
  def generate_article(self, title: str, header_point_map: dict) -> str:
    """Generates three talking points for the outline"""
    ...
    prompt = f"""Generate a technical article with title {title} in an informative tone.
...
if __name__ == "__main__":
  with Steamship.temporary_workspace() as client:
    ...
    headers = generator.generate_outline(title, tags, tone)
    ...
    for header in headers_list:
      talking_points = generator.generate_talking_points(header, tone)
      header_talking_points_map[header] = response_to_list(talking_points)

    # generate and save article
    article = generator.generate_article(title, header_talking_points_map, tone)
